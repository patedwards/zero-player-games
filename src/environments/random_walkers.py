...
class RandomWalkers(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 4}

    def __init__(self, n_walkers, render_mode=None, controller=OOPController):
        self.controller = controller()

        # add n_walkers random walkers to the simulation
        for _ in range(n_walkers):
            self.controller.report_position(RandomWalker(controller=self.controller))

        self.window_size = 256  # Size of the PyGame window
